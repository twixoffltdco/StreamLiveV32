...
if ($stmt->fetch()) {
  db()->prepare('DELETE FROM channel_likes WHERE channel_id = ? AND user_id = ?')->execute([$channelId, $user['id']]);
  $liked = false;
} else {
  db()->prepare('INSERT IGNORE INTO channel_likes (channel_id, user_id) VALUES (?, ?)')->execute([$channelId, $user['id']]);
  $liked = true;
}
