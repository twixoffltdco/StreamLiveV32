...
if ($profileUser) {
  ...
  $optionalCols = [
    ...
    'is_brand', 'brand_owner_id', 'brand_verified',
  ];
...
<div class="pg-wrap">
  ...
  <div class="pg-hero">
    ...
    <div class="pg-hero-main">
      <?php $__vibeAv = function_exists('vibe_nick_frame_class') ? vibe_nick_frame_class($profileUser) : 'vibe-frame-sept3'; if ($__vibeAv === '') $__vibeAv = 'vibe-frame-sept3'; ?>
      <img class="pg-avatar <?= e($__vibeAv) ?>" src="<?= e($avatarUrl) ?>" alt="" width="96" height="96" onerror="this.onerror=null;this.src='https://www.gravatar.com/avatar/?d=identicon&s=96'">
      ...
      <div class="pg-name-row">
        ...
        <div class="pg-name-cluster">
          ...
          <?php if (!empty($profileUser['is_brand'])): ?>
            <span title="Бренд-аккаунт StreamLive" style="display:inline-block;margin-left:6px;padding:2px 8px;border-radius:999px;font-size:11px;font-weight:800;background:#022c22;color:#4ade80;vertical-align:middle">✓ Бренд</span>
          <?php endif; ?>
        </div>
      ...
      <?php
        // Пометка: бренд-аккаунт + кто создал
        if (!empty($profileUser['is_brand'])) {
          if (!function_exists('brands_owner_username') && is_file(__DIR__ . '/includes/brands.php')) {
            require_once __DIR__ . '/includes/brands.php';
          }
          $ownName = function_exists('brands_owner_username')
            ? brands_owner_username((int)$profileUser['id'])
            : '';
          echo '<div class="pg-sub" style="opacity:.9;margin-top:6px;padding:8px 10px;border-radius:10px;background:rgba(0,162,255,.08);border:1px solid rgba(0,162,255,.2)">';
          echo '✓ <b>Бренд-аккаунт StreamLive</b>.';
          if ($ownName !== '') {
            echo ' Создан пользователем <a href="/profile.php?username=' . htmlspecialchars(urlencode($ownName), ENT_QUOTES) . '"><b>@' . htmlspecialchars($ownName, ENT_QUOTES) . '</b></a>.';
          }
          echo '</div>';
        }
      ?>
      <?php if (!empty($isOwnProfile) && empty($profileUser['is_verified']) && empty($profileUser['is_brand'])): ?>
...
<div class="pg-actions">
  ...
  <!-- ачивки ВНЕ hero — не выталкивают аватар -->
  <?php
        if (function_exists('vibe_check_time_based')) { try { vibe_check_time_based($profileUser); } catch (Throwable $e) {} }
        if (function_exists('vibe_render_achievements')):
      ?>
        <div class="pg-glass vibe-ach-box" style="margin:12px auto 0;max-width:420px;text-align:left;padding:10px 12px">
          <div style="font-size:12px;font-weight:700;margin-bottom:6px;opacity:.85">Ачивки</div>
          <?= vibe_render_achievements((int)$profileUser['id']) ?>
        </div>
      <?php endif; ?>
