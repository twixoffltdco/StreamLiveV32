...
<div class="container" style="max-width:980px">
  ...
  <p class="res-author-row" style="color:var(--text-dim);display:flex;flex-wrap:wrap;align-items:center;gap:8px">Автор:
    <?php
      $author = ['id' => (int)($r['author_id'] ?? $r['user_id'] ?? 0), 'username' => $r['username'] ?? '', 'username_css' => $r['username_css'] ?? null, 'prefix_id' => $r['prefix_id'] ?? null, 'custom_prefix_id' => $r['custom_prefix_id'] ?? null, 'is_verified' => $r['is_verified'] ?? 0, 'nick_decor_url' => $r['nick_decor_url'] ?? null, 'nick_decor_pos' => $r['nick_decor_pos'] ?? null, 'avatar' => $r['avatar'] ?? null, 'gravatar_email' => $r['gravatar_email'] ?? null];
      if (function_exists('render_user_badge')) {
        echo render_user_badge($author, 22);
      } else {
        echo '<a href="/profile?username=' . e($r['username']) . '">' . e($r['username']) . '</a>';
      }
    ?>
    · просмотров: <?= (int)$r['views'] ?> · <?= e($r['created_at']) ?>
  </p>
