    ...
    <?php foreach (array_reverse($posts) as $p): ?>
      <div class="msg-bubble-row" data-id="<?= (int)$p['id'] ?>">
        <div class="msg-bubble" style="max-width:80%">
          <div class="bc-author" style="margin-bottom:6px"><?php
            $author = $p;
            $author['id'] = (int)($p['user_id'] ?? $p['author_id'] ?? 0);
            // не трогаем $p['id'] — это id ПОСТА (реакции/комменты)
            echo function_exists('render_user_badge') ? render_user_badge($author, 24) : e($p['username'] ?? '');
          ?></div>
