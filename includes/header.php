...
<style id="sl-theme-hint-css">
  ...
  <link rel="stylesheet" href="/assets/css/vibe.css?v=20260809fix">
  <script src="/assets/js/vibe.js?v=20260809fix" defer></script>
  <link rel="stylesheet" href="/assets/css/profile-mobile-avatar.css?v=20260811m">
</head>
...
<?php
$__vibeFrameCls = 'vibe-frame-sept3';
try {
  if (is_file(__DIR__ . '/vibe.php')) {
    require_once __DIR__ . '/vibe.php';
    $__vf = function_exists('vibe_active_frame') ? vibe_active_frame() : null;
    if (!empty($__vf['class'])) $__vibeFrameCls = $__vf['class'];
  }
} catch (Throwable $e) {}
?>
<body data-vibe-frame="<?= htmlspecialchars($__vibeFrameCls ?? 'vibe-frame-sept3', ENT_QUOTES) ?>"<?= !empty($__light) ? ' class="light-mode" data-pl-skin="light"' : ' data-pl-skin="dark"' ?>>
...
<nav class="navbar">
  ...
  <div class="nav-links">
    ...
    <?php if ($__user): ?>
      ...
      <?php
        // StreamLive бренды — переключатель личного / бренд-аккаунта
        if (is_file(__DIR__ . '/brand_switcher_widget.php')) {
          try { include __DIR__ . '/brand_switcher_widget.php'; } catch (Throwable $e) {}
        }
      ?>
...
<?php endforeach; ?>
<?php
// Баннер партнёрской подписки (другие баннеры не убираем)
if (!empty($__user) && empty($__user['is_brand'])) {
  try {
    if (is_file(__DIR__ . '/partners.php')) {
      require_once __DIR__ . '/partners.php';
      $__psub = function_exists('partners_active_sub') ? partners_active_sub((int)$__user['id']) : null;
      if ($__psub && !empty($__psub['access_until'])) {
        $__untilFmt = date('d.m.Y H:i', strtotime((string)$__psub['access_until']));
        echo '<div class="container" style="margin-top:10px"><div class="alert" style="background:linear-gradient(90deg,rgba(167,139,250,.18),rgba(0,162,255,.12));border:1px solid rgba(167,139,250,.35);border-radius:12px;padding:12px 16px;display:flex;flex-wrap:wrap;gap:10px;align-items:center;justify-content:space-between">';
        echo '<div style="font-size:13.5px;line-height:1.45"><b>Платная подписка активна</b> до <b>' . htmlspecialchars($__untilFmt, ENT_QUOTES, 'UTF-8') . '</b>.';
        echo ' Лимит бренд-аккаунтов — до 50. Если хотите продлить — ищите промокод у <a href="/partners_welcome.php" style="font-weight:700">партнёров StreamLive</a>.</div>';
        echo '<a href="/partners/" style="font-size:12px;font-weight:800;white-space:nowrap;text-decoration:none;padding:6px 10px;border-radius:8px;background:#334155;color:#fff">Кабинет</a>';
        echo '</div></div>';
      }
    }
  } catch (Throwable $e) {}
