 ...
 * Активация промокода = доступ на 3 дня, потом снова ввод кода.
 * 
...
function paid_ensure_schema(): void {
  ...
  try {
    db()->exec(
      "CREATE TABLE IF NOT EXISTS promo_activations (
        ...
        access_until DATETIME NULL COMMENT 'конец доступа (3 дня)',
  ...
  try {
    // старые активации без срока → 3 дня от activated_at
    db()->exec("UPDATE promo_activations SET access_until = DATE_ADD(activated_at, INTERVAL 3 DAY) WHERE access_until IS NULL AND is_revoked = 0");
  } catch (Throwable $e) {}
...
function paid_user_can_watch(?array $user, array $channel): bool {
  ...
  // Партнёрская подписка (30 дн.) — доступ ко всем платным каналам.
  // Проверяем реальный личный id (не brand act-as).
  $realUid = (int)($_SESSION['user_id'] ?? $uid);
  try {
  ...
  try {
    ...
    // Активации channel-промо тоже на личный id
    $st->execute([$realUid, $cid, $cid]);
...
function paid_activate_code(int $userId, string $code, int $channelId, bool $acceptTerms): array {
  ...
  try {
    ...
    $until = date('Y-m-d H:i:s', time() + 3 * 86400); // 3 дня
    db()->prepare(
    ...
    return [
      'ok' => true,
      'message' => 'Доступ открыт на 3 дня (до ' . date('d.m.Y H:i', strtotime($until)) . ')',
      'access_until' => $until,
    ];
  } catch (Throwable $e) {
  ...
  <div class="container" style="max-width:480px;padding:40px 16px">
    <div class="form-card">
      ...
      <p style="color:var(--text-dim);font-size:14px">
        ...
        После активации доступ действует <b>3 дня</b>, затем нужно снова ввести код.
      </p>
      ...
      <?php else: ?>
        <form method="post">
          ...
          <label style="display:flex;gap:8px;align-items:flex-start;margin:14px 0;font-weight:400;font-size:13px">
            ...
            <span>Принимаю условия. Администрация/модераторы могут отозвать доступ. Через 3 дня доступ истекает.</span>
          </label>
          <button class="btn btn-primary" type="submit">Активировать на 3 дня</button>
        </form>
      <?php endif; ?>
    </div>
  </div>
