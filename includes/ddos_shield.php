...
function ddos_concurrency_guard(): ?string {
  ...
  // Чистим маркеры старше 10 сек (застрявшие от упавших/убитых хостингом процессов) —
  // иначе счётчик будет только расти и в итоге заблокирует вообще всех, включая настоящих
  // гостей. Раньше порог был 30 сек — на медленном бесплатном хостинге это слишком долго:
  // обычный PHP-запрос отрабатывает за доли секунды, и лок, переживший 10 секунд, почти
  // наверняка брошен (процесс убит хостером по таймауту/памяти), а не реально ещё работает.
  foreach (glob(DDOS_LOCK_DIR . '/*.lock') ?: [] as $file) {
    if (time() - filemtime($file) > 10) @unlink($file);
  }
...
function ddos_guard(): void {
  ...
  //
  // ИСКЛЮЧЕНИЕ для известных краулеров/ИИ-агентов (GPTBot, ChatGPT-User, PerplexityBot и
  // т.д.): они принципиально не выполняют JavaScript, поэтому JS/PoW-проверка для них не
  // "подождите секунду", а "страница никогда не откроется". Человек с настоящим браузером
  // проходит эту же проверку незаметно за доли секунды.
  if (!is_known_crawler_ua((string)($_SERVER['HTTP_USER_AGENT'] ?? '')) && ddos_under_attack_mode_enabled() && !ddos_verify_pow_cookie()) {
