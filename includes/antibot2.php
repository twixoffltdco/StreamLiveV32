...
function antibot_exempt_paths(): array {
  return [
    ...
    '/broadcast_post_comments.php', '/broadcast_post_comment_add.php',
...
function antibot_ensure_table(): void {
  ...
  // block_count — новая колонка для эскалации (см. antibot_guard). На проектах, где таблица
  // уже была создана раньше без неё, добавляем отдельным ALTER. Пробуем максимум один раз за
  // время жизни приложения (флаг в settings), а не на каждый гостевой запрос — иначе лишняя
  // попытка ALTER на каждый хит будет зря грузить БД после первого же успеха/неудачи.
  static $checked = false;
  if ($checked) return;
  $checked = true;
  try {
    if (get_setting('antibot_schema_v2') === '1') return;
    try {
    ...
    set_setting('antibot_schema_v2', '1');
