...
$stmt = db()->prepare(
  "SELECT bp.id, bp.body, u.username FROM broadcast_posts bp JOIN users u ON u.id = bp.author_id
   WHERE bp.channel_id = ? AND bp.id > ? AND bp.is_deleted = 0 ORDER BY bp.id ASC LIMIT 50"
);
$stmt->execute([$channelId, $after]);
$rows = $stmt->fetchAll();

// Пост долетел до открытой вкладки зрителя через поллинг — считаем это просмотром,
// точно так же, как и при обычной загрузке страницы (broadcast_channel.php). Отмечаем
// ДО подсчёта счётчиков ниже, чтобы цифра в ответе уже была верной, без ручных поправок.
if ($rows) {
...
echo json_encode(['posts' => array_map(function ($r) {
  return [
    'id' => (int)$r['id'], 'username' => $r['username'], 'body' => $r['body'],
    'views_count' => $r['views_count'], 'comments_count' => $r['comments_count'],
  ];
}, $rows)]);
