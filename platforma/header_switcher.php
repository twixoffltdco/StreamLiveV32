...
if (!in_array($pl_mode, ['streamlife', 'platforma', 'telegram', 'prohub', 'flex'], true)) {
  $pl_mode = 'streamlife';
}
...
.pl-mode-switch a.on-tg{background:#2AABEE;color:#fff}.pl-mode-switch a.on-ph{background:#3b82f6;color:#fff}.pl-mode-switch a.on-fx{background:#00a2ff;color:#fff}
...
<?php elseif ($pl_mode === 'flex'): ?>
<link rel="stylesheet" href="/platforma/theme-flex.css?v=4">
<script>
window.PL_FLEX = {
  logged: <?= $pl_logged ? 'true' : 'false' ?>,
  id: <?= (int)$pl_uid ?>,
  name: <?= json_encode($pl_username ?: 'Guest', JSON_UNESCAPED_UNICODE) ?>
};
try{
  document.documentElement.classList.add('pl-theme-flex');
  document.addEventListener('DOMContentLoaded',function(){
    if(!document.body) return;
    document.body.classList.add('pl-theme-flex');
  });
}catch(e){}
</script>
<script src="/platforma/theme-flex.js?v=14" defer></script>
<?php endif; ?>
